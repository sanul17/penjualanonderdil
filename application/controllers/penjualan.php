...
//    DELETE
	...
	function update($id){
		...
		$data['data_barang'] = $this->app_model->getBarangJual()->result();
		$data['data_penjualan'] = $this->app_model->getSelectedData("tbl_penjualan", $update);
		foreach($data['data_penjualan']->result() as $dph)
		{
			$sess_data['nama_pelanggan'] = $dph->nama_pelanggan;
			$data['nama_user'] = $this->app_model->getNamaSales($dph->kd_user);
			$this->session->set_userdata($sess_data);
		}

		$data['data_penjualan_detail'] = $this->app_model->manualQuery("select a.kd_penjualan, a.kd_barang, a.qty, b.nama_barang, b.harga from tbl_penjualan_detail a left join tbl_barang b 
			on a.kd_barang=b.kd_barang where a.kd_penjualan='".$update['kd_penjualan']."'");
		...
		if (!empty($cek)) {
			...
			$this->form_validation->set_rules('nama_pelanggan', 'Nama Pelanggan', 'required');
			if ($this->form_validation->run()) {
				$id_update['kd_penjualan'] = $this->input->post('kd_penjualan');
				$update['nama_pelanggan'] = $this->input->post('nama_pelanggan');
				$update['kd_user'] = $this->input->post('kd_user');
				...
				foreach($this->cart->contents() as $items)
				{
					$update_detail['kd_penjualan'] = $this->input->post('kd_penjualan');
	...
	function addStok($id){
		$dt['title']='Toko Onderdil | Tambah penjualan';
		$update['kd_penjualan'] = $id;
		$cek = $this->session->userdata('logged_in');
		$result = $this->app_model->getSelectedData('tbl_penjualan', $update)->result();
		foreach ($result as $key => $value) {
			$data['kd_penjualan'] = $value->kd_penjualan;
			$data['nama_penjualan'] = $value->nama_penjualan; 
			$data['min_stok'] = $value->min_stok; 
			$data['stok'] = $value->stok; 
		}
		if (!empty($cek)) {
			$this->form_validation->set_error_delimiters('<div class="text-red"> <i class="fa fa-ban"></i>  ', ' </div>');
			$this->form_validation->set_rules('kd_penjualan', 'kd_penjualan', 'required');
			$this->form_validation->set_rules('nama_penjualan', 'Nama penjualan', 'required');
			...
			if(isset($id)){
				if ($this->form_validation->run()) {
					...
					$update['nama_penjualan'] = $this->input->post('nama_penjualan');
					$update['min_stok'] = $this->input->post('min_stok'); 
					$stok = $this->input->post('stok');
					$update['stok'] = $stok+$this->input->post('qty');
					if ($this->app_model->updateData('tbl_penjualan', $update, $id_update)) {
						$pesan = 'Tambah Stok penjualan Sukses';
						$this->session->set_flashdata('pesan', $pesan);
						redirect(base_url('penjualan'));
					}else{
						$data['pesan'] = 'Tambah Stok penjualan Gagal';
						$this->load->view('elements/header', $dt);
